...
function generateDisaster(seed){
  ...
  const rint=(a,b)=>Math.floor(a+rnd()*(b-a+1));
  const clamp=(v,lo,hi)=>Math.max(lo,Math.min(hi,v));
  const W=1120, H=2600;                 // bigger arena so the buttons can spread out
  const platforms=[]; let id=0;
  platforms.push({id:id++, x:0, y:H-30, w:W, h:30, type:'big'});          // ground
  const start={x:W/2, y:H-30-40};
  // a single guaranteed-climbable staircase that zig-zags across the wide map
  const path=[]; let prevX=W/2, y=H-30;
  for(let i=0;i<24;i++){
    y -= rint(82,98);
    const w=rint(104,140);
    const off=rint(48, w>=130?120:106);
    let dir=rnd()<0.5?-1:1, nx=prevX+dir*off;
    if(nx<90 || nx>W-90) nx=prevX-dir*off;
    nx=clamp(nx, 90+w/2, W-90-w/2);
    prevX=nx;
  ...
  // side ledges branching off the path — spread wide across the bigger map
  const ledges=path.slice();
  for(let i=0;i<34;i++){
    const base=path[rint(1, path.length-1)];
    const sw=rint(92,120);
    const sx=clamp((base.x+base.w/2) + (rnd()<0.5?-1:1)*rint(96,150) - sw/2, 40, W-40-sw);
    const sp={id:id++, x:sx, y:base.y - rint(0,40), w:sw, h:22, type:'normal'};
    platforms.push(sp); ledges.push(sp);
  }
  // 20 disaster buttons scattered on the ledges
  for(let i=ledges.length-1;i>0;i--){ const j=Math.floor(rnd()*(i+1)); const t=ledges[i];ledges[i]=ledges[j];ledges[j]=t; }
  for(let i=0;i<20 && i<ledges.length;i++){
    const L=ledges[i];
