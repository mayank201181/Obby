/* ===== AREA CLASH 🏰 =====
   ...
   ball moves, picks, drops, tags and the win all sync over the room. */
...
function initClash(){
  ...
  const hostSide = !mp || MP.isHost;          // solo & host = LEFT castle
  const zMe  = hostSide ? CLASH.ZONE_L : CLASH.ZONE_R;
  const zFoe = hostSide ? CLASH.ZONE_R : CLASH.ZONE_L;
  Game.world.start = { x: hostSide?220:1980, y: CLASH.FLOOR_TOP-40 };
  const meBallX  = hostSide ? 200 : 2000;
  const foeBallX = hostSide ? 1980 : 220;
  const C = Game.clashC = {
    phase:'build', buildUntil:t+(Game.clashBuildMs||60000),
    mp, hostSide, zMe, zFoe,
    bricks:[], cannons:[], traps:[], shots:[], fx:[],
    myBall:{x:meBallX, y:CLASH.FLOOR_TOP-14, hx:meBallX, hy:CLASH.FLOOR_TOP-14, carrier:null},
    foeBall:{x:foeBallX, y:CLASH.FLOOR_TOP-14, hx:foeBallX, hy:CLASH.FLOOR_TOP-14, carrier:null},
    ...
    gunCdUntil:0, swordCdUntil:0, tagCdUntil:0,
    botBuildT:0, botPlan:null, hudTxt:'', actTxt:'',
  };
  if(!mp){
  ...
  toast(mp ? '🏰 BUILD! Your friend is building too — hide your 🏀!'
           : '🏰 BUILD! Make your castle & hide your 🏀 (tap the bar below)');
  renderClashBar();
  const ab=document.getElementById('clashActBtn'); if(ab) ab.style.display='none';
}

function clashSend(d){
  if(Game.multiplayer && typeof mpSend==='function')
    mpSend(Object.assign({t:'clash', from:MP.selfId}, d));
}
/* a clash message from the friend in the room */
function onClashNet(msg){
  const C=Game.clashC; if(!C) return;
  switch(msg.a){
    case 'brick':  clashAddBrick(C,{x:msg.x, y:msg.y, col:msg.col, mine:false}); break;
    case 'cannon': C.cannons.push({x:msg.x, y:msg.y, mine:false, lastFire:0}); break;
    case 'trap':   C.traps.push({x:msg.x, y:msg.y, mine:false}); break;
    case 'ball':   if(!C.foeBall.carrier){ C.foeBall.x=C.foeBall.hx=msg.x; C.foeBall.y=C.foeBall.hy=msg.y; } break;
    case 'dmg': {  const b=C.bricks.find(bb=>bb.x===msg.x&&bb.y===msg.y); if(b) b.dmg=(b.dmg||0)+1; break; }
    case 'rm':  {  const b=C.bricks.find(bb=>bb.x===msg.x&&bb.y===msg.y); if(b) clashRemoveBrick(C,b); break; }
    case 'rmcannon': { const i=C.cannons.findIndex(c=>Math.abs(c.x-msg.x)<2&&Math.abs(c.y-msg.y)<2); if(i>=0) C.cannons.splice(i,1); break; }
    case 'rmtrap':   { const i=C.traps.findIndex(c=>Math.abs(c.x-msg.x)<2&&Math.abs(c.y-msg.y)<2); if(i>=0) C.traps.splice(i,1); break; }
    case 'shot':   C.shots.push({x:msg.x, y:msg.y, vx:msg.vx, vy:msg.vy, cosmetic:true, born:Game.t}); break;
    case 'pick':   C.myBall.carrier='foe'; toast('😱 They grabbed your ball — TAG THEM!'); SFX.hit(); break;
    case 'drop':   C.myBall.carrier=null; C.myBall.x=C.myBall.hx; C.myBall.y=C.myBall.hy;
                   toast('🖐️ You tagged them — your ball zoomed home!'); SFX.rare(); break;
    case 'tagreq': if(C.foeBall.carrier==='me'){
                     C.foeBall.carrier=null; C.foeBall.x=C.foeBall.hx; C.foeBall.y=C.foeBall.hy;
                     Game.stunUntil=Math.max(Game.stunUntil||0, Game.t+1000);
                     addShake(4); SFX.hit(); toast('😱 Tagged! Their ball went back!');
                     clashSend({a:'drop'});
                   } break;
    case 'win':    clashEnd(false, true); break;
  }
}
...
function renderClashBar(){
  ...
  if(C.phase==='build'){
    ...
    bar.innerHTML =
      btn('sword','🗡️ Sword (2 hits a brick)', C.weapon==='sword') +
...
function clashBuildAt(C,x,y){
  if(!inZone(C.zMe,x) || y>CLASH.FLOOR_TOP){ toast('Build inside YOUR zone!'); return; }
  ...
  if(C.buildSel==='erase'){
    if(hit){ clashRemoveBrick(C,hit); clashSend({a:'rm', x:hit.x, y:hit.y}); }
    else { const ci=C.cannons.findIndex(cn=>cn.mine&&Math.abs(cn.x-x)<30&&Math.abs(cn.y-y)<30);
           if(ci>=0){ const cn=C.cannons[ci]; C.cannons.splice(ci,1); clashSend({a:'rmcannon', x:cn.x, y:cn.y}); }
           const ti=C.traps.findIndex(tr=>tr.mine&&Math.abs(tr.x-x)<30&&Math.abs(tr.y-y)<30);
           if(ti>=0){ const tr=C.traps[ti]; C.traps.splice(ti,1); clashSend({a:'rmtrap', x:tr.x, y:tr.y}); } }
    return;
  }
  if(C.buildSel==='brick'){
    if(hit) return;
    clashAddBrick(C,{x:gx, y:Math.max(gy, 40), col:C.buildColor, mine:true});
    clashSend({a:'brick', x:gx, y:Math.max(gy,40), col:C.buildColor});
    SFX.click(); return;
  }
  ...
  if(C.buildSel==='ball'){
    ...
    clashSend({a:'ball', x:C.myBall.hx, y:C.myBall.hy});
    toast('🏀 Ball hidden here — shh!'); SFX.coin(); return;
  }
}
function clashAddBrick(C,b){
  const brick={id:90000+C.bricks.length, type:'brick', x:b.x, y:b.y, w:CLASH.GRID, h:CLASH.GRID, col:b.col, mine:!!b.mine, dmg:0};
...
/* one sword/bot swing on an enemy brick: 2 hits total to smash it */
function clashHitBrick(C,b){
  b.dmg=(b.dmg||0)+1;
  if(b.dmg>=2){
    clashRemoveBrick(C,b); clashSend({a:'rm', x:b.x, y:b.y});
    return true;
  }
  C.fx.push({e:'💢', x:b.x+20, y:b.y+20, vy:-0.5, life:1});
  clashSend({a:'dmg', x:b.x, y:b.y});
  return false;
}
function clashRemote(){ return Game.multiplayer ? (mpRemoteList().find(r=>typeof r.x==='number')||null) : null; }
...
function clashFightAt(C,x,y){
  ...
  if(C.weapon==='breach'){
    ...
    clashRemoveBrick(C,b); clashSend({a:'rm', x:b.x, y:b.y}); addShake(4); SFX.hit(); return;
  }
  if(C.weapon==='gun'){
    ...
    clashSend({a:'shot', x:Math.round(px), y:Math.round(py), vx:+(dx/d*10).toFixed(2), vy:+(dy/d*10).toFixed(2)});
    SFX.jump(); return;
  }
  // sword: swing — bonks the enemy if close, or chips an enemy brick (2 hits)
  ...
  if(!Game.multiplayer){
    ...
    if(Math.hypot(b.x+17-px, b.y+17-py)<85){
      b.stunUntil=Game.t+1500; b.vx=p.facing*6; addShake(3); SFX.hit(); toast('🗡️ Bonk!'); return;
    }
  } else {
    const r=clashRemote();
    if(r && Math.hypot(r.x+17-px, r.y+17-py)<85){
      if(typeof mpSendStun==='function') mpSendStun(r.id, 1.5);
      addShake(3); SFX.hit(); toast('🗡️ Bonk!'); return;
    }
  }
  const b2=C.bricks.find(bb=>!bb.mine && x>=bb.x&&x<=bb.x+bb.w&&y>=bb.y&&y<=bb.y+bb.h);
  if(b2 && Math.hypot(b2.x+20-px, b2.y+20-py)<120){
    const smashed=clashHitBrick(C,b2);
    addShake(3); SFX.hit();
    toast(smashed ? '🗡️ SMASHED through!' : '🗡️ Crack! One more hit…');
  }
}
...
function updateClash(dt){
  ...
  clashSetHud(C);
  clashSolid(p);
  // stay on your own side while building
  if(C.phase==='build'){
    if(C.hostSide && p.x>1000){ p.x=1000; p.vx=0; }
    if(!C.hostSide && p.x<1200-p.w){ p.x=1200-p.w; p.vx=0; }
  }

  if(!C.mp) clashBot(C,dt);

  // cannons: my screen simulates ENEMY cannons vs me (and mine vs the bot in solo)
  for(const cn of C.cannons){
    if(C.phase!=='battle') break;
    if(C.mp && cn.mine) continue;                     // my cannons run on my friend's screen
  ...
  // shots
  const remote=C.mp?clashRemote():null;
  for(let i=C.shots.length-1;i>=0;i--){ const s=C.shots[i];
    ...
    if(s.cosmetic) continue;                          // friend's pellets: just visuals here
    if(s.mine){
      if(C.mp){ if(remote && s.x>remote.x&&s.x<remote.x+34&&s.y>remote.y&&s.y<remote.y+34){
                  if(typeof mpSendStun==='function') mpSendStun(remote.id, 1); C.shots.splice(i,1); SFX.hit(); } }
      else { const b=C.bot;
  ...
  // traps: enemy traps trigger on ME locally (my traps on the bot in solo)
  for(let i=C.traps.length-1;i>=0;i--){ const tr=C.traps[i];
    if(C.phase!=='battle') break;
    if(tr.mine){
      if(!C.mp){ const b=C.bot;
      ...
      Game.stunUntil=Math.max(Game.stunUntil||0,t+2000); C.traps.splice(i,1);
      clashSend({a:'rmtrap', x:tr.x, y:tr.y});
      C.fx.push({e:'🪤',x:tr.x,y:tr.y-14,vy:-1,life:1}); addShake(4); SFX.hit(); toast('🪤 Trapped! Stunned for 2s!');
    }
  }
  // carrying + tags
  if(C.foeBall.carrier==='me'){ C.foeBall.x=p.x+p.w/2; C.foeBall.y=p.y-16; }
  if(!C.mp && C.myBall.carrier==='bot'){ C.myBall.x=C.bot.x+17; C.myBall.y=C.bot.y-14; }
  if(C.mp && C.myBall.carrier==='foe' && remote){ C.myBall.x=remote.x+17; C.myBall.y=remote.y-16; }
  if(C.phase==='battle'){
    if(!C.mp){
      const touching = Math.hypot(C.bot.x+17-(p.x+p.w/2), C.bot.y+17-(p.y+p.h/2)) < 42;
      if(touching){
        if(C.myBall.carrier==='bot'){
        ...
        } else if(C.foeBall.carrier==='me' && t>=(C.bot.stunUntil||0)){
          C.foeBall.carrier=null; C.foeBall.x=C.foeBall.hx; C.foeBall.y=C.foeBall.hy;
          Game.stunUntil=Math.max(Game.stunUntil||0,t+1000);
          addShake(4); SFX.hit(); toast('😱 Tagged! Their ball went back!');
        }
      }
    } else if(remote && C.myBall.carrier==='foe' && t>=C.tagCdUntil &&
              Math.hypot(remote.x+17-(p.x+p.w/2), remote.y+17-(p.y+p.h/2)) < 46){
      C.tagCdUntil=t+800;
      clashSend({a:'tagreq'});          // the carrier's client confirms & drops
    }
  }
  clashActionBtn(C);
...
function clashSetHud(C){
  ...
  } else txt = C.foeBall.carrier==='me' ? '🏀 RUN HOME!'
             : (C.myBall.carrier==='bot'||C.myBall.carrier==='foe') ? '😱 They have YOUR ball!'
             : '⚔️ Steal their 🏀!';
...
function clashActionBtn(C){
  ...
  if(C.phase==='battle'){
    if(C.foeBall.carrier==='me' && inZone(C.zMe, p.x+p.w/2)) txt='⬇ Put down — WIN!';
...
function clashAction(){
  ...
  const p=Game.player;
  if(C.foeBall.carrier==='me' && inZone(C.zMe, p.x+p.w/2)){ clashEnd(true); return; }
  if(!C.foeBall.carrier && Math.hypot(C.foeBall.x-(p.x+p.w/2), C.foeBall.y-(p.y+p.h/2))<70){
    C.foeBall.carrier='me'; clashSend({a:'pick'}); SFX.coin(); toast('🏀 GOT IT — run home!');
  }
}
...
function clashBot(C,dt){
  ...
  if(C.phase==='build'){
    ...
    if(C.botBuildT>1.1 && C.botPlan.length){
      ...
      else if(piece.k==='cannon') C.cannons.push({x:piece.x+20, y:piece.y+20, mine:false, lastFire:0});
  ...
  const oldX=b.x;
  clashBotPhysics(b);
  // blocked by a wall? jump; still stuck? whack a brick like a player (2 hits)
  if(Math.abs(b.x-oldX)<0.4){
    ...
    if(C.blockedT>1.4 && t-b.breachAt>1600){
      ...
      if(brick){ b.breachAt=t;
        const smashed=clashHitBrick(C,brick); SFX.hit();
        toast(smashed ? '💥 The bot smashed one of your bricks!' : '💢 The bot cracked one of your bricks!'); }
      C.blockedT=0;
    }
  } else C.blockedT=0;
}
...
/* ---------- win / lose ---------- */
function clashEnd(won, fromNet){
  if(Game.finished) return;
  Game.finished=true;
  const ab=document.getElementById('clashActBtn'); if(ab) ab.style.display='none';
  const cb=document.getElementById('clashBar'); if(cb) cb.style.display='none';
  if(won){ addCoins(CLASH.WIN_COINS); SFX.win(); if(!fromNet) clashSend({a:'win'}); }
  else SFX.hit();
  if(Game.onLevelComplete) Game.onLevelComplete(1, won?CLASH.WIN_COINS:0, false, {clash:true, survived:won, mp:Game.multiplayer});
}
...
function drawClash(ctx){
  ...
  ctx.fillStyle='#74a8ff'; ctx.fillRect(C.zMe.x0,80,C.zMe.x1-C.zMe.x0,CLASH.FLOOR_TOP-80);
  ctx.fillStyle='#ff8f8f'; ctx.fillRect(C.zFoe.x0,80,C.zFoe.x1-C.zFoe.x0,CLASH.FLOOR_TOP-80);
  ctx.globalAlpha=1;
  ctx.font='bold 22px Nunito'; ctx.textAlign='center'; ctx.fillStyle='rgba(90,80,140,.5)';
  ctx.fillText('🏠 YOUR CASTLE', (C.zMe.x0+C.zMe.x1)/2, 120);
  ctx.fillText('🏰 THEIR CASTLE', (C.zFoe.x0+C.zFoe.x1)/2, 120);
  ctx.textBaseline='middle';
  // cracked bricks show a 💢
  ctx.font='18px serif';
  for(const b of C.bricks) if(b.dmg) ctx.fillText('💢', b.x+20, b.y+20);
